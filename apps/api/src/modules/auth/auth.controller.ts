...
export class AuthController {
  constructor(
    ...
    public readonly firmRepo: Repository<FirmEntity>,
  ...
  @Post('/register')
  async register(@Body() { firstName, lastName, password, tokenId, timezone }: RegistrationBody): Promise<SuccessDTO> {
    const { email, firmId, firmPermission, lobbymaticPermission } = await this.registerTokenRepo.findOneByOrFail({ id: tokenId });
    await this.registerTokenRepo.delete(tokenId);
    const user = await this.userRepo.save(
      this.userRepo.create({ firstName, lastName, email, password, firmId, firmPermission, lobbymaticPermission, timezone }),
    );
  ...
  async sendSignupEmail(
    ...
    @Body() { email, firmId, firmPermission, lobbymaticPermission }: SignupEmailBody,
  ): Promise<SuccessDTO> {
    if (lobbymaticPermission === ELobbymaticPermission.Admin && passport.lobbymaticPermission !== ELobbymaticPermission.Admin) {
      throw new Error('Nope!');
    }
    if (!lobbymaticPermission && !firmPermission) {
      throw new Error('No Permission Set');
    }
    if (await this.userRepo.findOneBy({ email })) {
      throw new Error('email already used');
    }
    const firm = await this.firmRepo.findOneByOrFail({ id: firmId });
    const registerToken = await this.registerTokenRepo.save({ email, firmId, firmPermission, lobbymaticPermission });
